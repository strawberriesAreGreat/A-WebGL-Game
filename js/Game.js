...
class Game {
  constructor() {
    ...
    if (!this.gl) {
      ...
      alert("Your browser doesn't support WebGl, please try again with an updated browser");
    }
...
drawBacteria() {
  ...
  for (var i = 0; i < this.bacteria.length; i++) {
    ...
    var n = this.initBuffers(circleOrgin, this.bacteria[i].size, this.bacteria[i].getR(), this.bacteria[i].getG(), this.bacteria[i].getB());
...
// pogggggers
  for (var i = 0; i < this.antiBacteria.length; i++) {
    ...
    var n = this.initBuffers(circleOrgin, this.antiBacteria[i].size, this.antiBacteria[i].getR(), this.antiBacteria[i].getG(), this.antiBacteria[i].getB());
    this.gl.drawArrays(this.gl.TRIANGLE_STRIP, 0, n);
...
initBuffers(orgin, scale, r, g, b) {
  ...
  this.gl.bufferData(this.gl.ARRAY_BUFFER, new Float32Array(vertices), this.gl.STATIC_DRAW); //sending from the CPU -> gpu memory once
